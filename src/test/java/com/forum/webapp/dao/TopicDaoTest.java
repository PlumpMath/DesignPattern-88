...
public class TopicDaoTest {
    ...
    public void createNewPrivateTopic() {
        ...
        assertThat(
                topics,
                Matchers.<Collection<TopicEntity>>allOf(
                        Matchers.hasSize(2),
                        Matchers.<TopicEntity>hasItem(Matchers.hasProperty("id",
                                Matchers.is(topicId)))));
        // The other users cannot.
        topics = topicDao.list(usersId[1]);
        assertThat(topics, Matchers.<Collection<TopicEntity>>allOf(
                Matchers.hasSize(1),
                Matchers.not(Matchers.<TopicEntity>hasItem(Matchers.hasProperty("id",
                        Matchers.is(topicId))))));
    }
